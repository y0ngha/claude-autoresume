...
_msg_en() { case "$1" in
  ...
  in_coreutils_get)  printf 'installing coreutils for daemon timeout protection (optional)...' ;;
  in_coreutils_skip) printf '• coreutils not installed — daemon runs without tmux-timeout protection (optional)' ;;
  ...
  in_not_zsh)     printf '⚠ login shell is not zsh (%%s). Shell functions are zsh-only — added to ~/.zshrc but will not load under bash.' ;;
...
_msg_ko() { case "$1" in
  ...
  ntf_idle_body)        printf "'%%s' 작업이 멈춤 (완료 또는 입력 대기)" ;;
  ntf_working_title)    printf 'Claude: 작업 시작' ;;
  ntf_working_body)     printf "'%%s' 생성 시작" ;;
  ntf_background_title) printf 'Claude: 백그라운드' ;;
  ntf_background_body)  printf "'%%s' 백그라운드 작업 중" ;;
  ...
  in_coreutils_get)  printf '데몬 타임아웃 보호용 coreutils 설치 중(선택)...' ;;
  in_coreutils_skip) printf '• coreutils 없음 — 데몬이 tmux 타임아웃 보호 없이 동작(선택)' ;;
  ...
  in_not_zsh)     printf '⚠ 로그인 셸이 zsh 가 아닙니다 (%%s). 셸 함수는 zsh 전용이라 ~/.zshrc 에 추가돼도 bash 에선 로드되지 않습니다.' ;;
