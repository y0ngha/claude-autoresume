...
- [The dashboard (csm)](#the-dashboard-csm)
- [How it detects limits and resumes](#how-it-detects-limits-and-resumes)
- [Turning auto-resume on and off per window](#turning-auto-resume-on-and-off-per-window)
- [Auto-approving permission prompts](#auto-approving-permission-prompts)
- [Per-state alerts](#per-state-alerts)
- [Configuration (`config.sh`)](#configuration-configsh)
- [Managing the daemon](#managing-the-daemon)
- [Files](#files)
- [`.zshrc` setup](#zshrc-setup)
...
It also installs coreutils through brew when neither `timeout` nor `gtimeout` is present, which lets the daemon give up on a wedged tmux call instead of hanging on it.
That one is optional and everything works without it.
...
Profile launchers like `claude-work` are covered in the [`.zshrc` setup](#zshrc-setup) below.
...
The screenshot at the top of this page is the default English UI.
Press `l` to switch to Korean.

Each row is one window and shows, in order, the window name, the profile, the state, the usage left, the last activity time, the last auto-resume time, and a marker when the window is excluded from auto-resume or from auto-approval.
The header line carries the daemon's auto-resume and auto-approve settings, and under the summary counts the last few auto-resume and auto-approve events from the log are listed.
...
A question that shows a preview panel next to its options is taller than the limit check's 20-line window, so approval reads a deeper `APPROVE_CAPTURE_LINES` window of its own.
Reading deeper means an already-answered dialog still sitting in the scrollback could be mistaken for a live one, so a dialog only counts as live when its footer is near the bottom of the screen and no reply has started below it.
...
| `APPROVE_CAPTURE_LINES` | 60 | bottom lines read for approval only. Dialogs with a preview panel are taller than `CAPTURE_LINES` |
...
- `disabled.list` — windows excluded from auto-resume, optional.
- `noapprove.list` — windows excluded from auto-approval only.
  Created by `y` in csm, optional.
- `lang` — the UI language setting, toggled by `l` in csm.
- `notify.conf` — the per-state alert setting, toggled by `t` in csm.
- `approve.conf` — the auto-approval setting, changed by `y` in csm.
